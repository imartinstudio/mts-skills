...
description: Use when analyzing, updating evidence for, or drafting an X suspension appeal specifically for account @iMartinstudio, including X emails, suspicious-login notices, locked/read-only access, or an inauthentic-behavior decision.
---
...
3. Use the Gmail connector first when current X notices matter. Search `from:(notify@x.com OR verify@x.com OR info@x.com) in:anywhere`; read the matching bodies before citing them.
...
3. Classify the current state: initial suspension, appeal pending, appeal denied, or a login-page remediation flow.
4. Identify the narrowest defensible explanation. Current evidence supports an automated inauthentic-behavior decision and anomalous-login signals; it does not identify a specific violating action.
5. Produce the requested output:
   - **Analysis:** timeline, evidence labels, uncertainty, and next safe step.
   - **Appeal draft:** concise English first, Chinese reference second; request manual review and specific violation details.
...
Use a factual, non-accusatory structure:
...
Do not repeat a generic VPN-only explanation after an appeal denial. Do not promise evidence that is unavailable. Do not state that a human review is guaranteed.
